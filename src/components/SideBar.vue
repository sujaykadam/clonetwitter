<template>
  ...
  <div class="hideOnMobile fixed h-full w-1/4 px-24 border-r border-lighter">
    ...
    <div v-if="isAuth" class="lg:w-full absolute bottom-2 ">
        ...
        <div v-if="dropdown === true" class="absolute bottom-0 left-0 w-64 rounded-lg shadow-md border-lightest bg-white mb-16">
          ...
          <button @click="logout" class="w-full text-left hover:bg-lightest border-t border-lighter p-3 test-sm focus:outline-none">
            Log out {{username}}
          </button>
        </div>
      </div>
      <button v-else class=" absolute bottom-2 text-white bg-blue rounded-full font-semibold focus:outline-none w-auto  h-12 w-52 p-3 hover:bg-darkblue">
        <p class="hidden lg:block">Log In</p>
        <i class="fa fa-plus lg:hidden"></i>
      </button>
